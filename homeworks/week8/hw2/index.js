...
// 也就是幫 onlaod 上 EventListener
  ...
  // 判斷 request 是否成功
    ...
    requestStreamFunction(top5[0].game.name)

    navbarList.addEventListener('click', (e) => {
      const gameName = e.target.innerText;
      requestStreamFunction(gameName)
    });
  } else {
...
function requestStreamFunction(target){
  document.querySelector('h1').innerText = target;
  // 更改 h1 標題
  document.querySelector('.streams').innerText = '';
  // 清空內容
  const requestStream = new XMLHttpRequest();
  // 每送一個請求就要多 new 一個 request
  requestStream.open('GET', `${apiSourse}streams?game=${encodeURIComponent(target)}&limit=20`, true);
